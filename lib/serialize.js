 ...
 */
const CLEANUP_MS = Number(process.env.SERIALIZER_RAW_TTL_MS) || 30_000;
...
export default class Serializer {
  ...
  serializeSync(msg) {
    ...
    // session-scoped cache (for compatibility with your other code)
    ...
    const extractQuoted = () => {
      ...
      return {
        type: qt,
        // keep minimal copy; this can be explicitly downloaded using download()
    ...
    // Create the message object (expose the same API as before)
    const msgObj = {
      raw: msg,
      mek: msg,
      client: conn,
    ...
    // helpers: botJid and botnum (preserve API but make robust)
    msgObj.botJid = () => {
      try {
    ...
    // The minimal cache may return adminIds + participantsCount instead of participants array.
    msgObj.loadGroupInfo = async () => {
      ...
      try {
        ...
        // support both shapes:
        // - md.participants => [{ id, isAdmin }, ...] (old)
        // - md.adminIds + participantsCount (compact)
        let participants = [];
        ...
        msgObj.groupParticipants = participants || [];
        ...
        // boolean flags (if absent in compact cache, default false)
    ...
    // ---------- group control helpers (preserved original implementations) ----------
    ...
    msgObj.updateJoinRequests = async (jids, action = "approve") => {
      try {
        const normalized = Array.isArray(jids) ? jids.map((j) => jidNormalizedUser(j)) : [jidNormalizedUser(jids)];
    ...
    // download media (keeps buffer local to caller)
    ...
    // send / react / reply helpers (preserve original semantics)
    msgObj.send = async (payload, options = {}) => {
      try {
        ...
        if (typeof payload === "string") {
          ...
          cend = { video: payload.video, caption: payload.caption || "", mimetype: payload.mimetype || "video/mp4" };
        } else if (payload.image) {
          cend = { image: payload.image, caption: payload.caption || "" };
        } else if (payload.audio) {
          cend = { audio: payload.audio, mimetype: payload.mimetype || "audio/mp4", ptt: payload.ptt || false };
        } else {
          cend = payload;
        }
        if (options.mentions) cend.mentions = options.mentions;
        if (options.edit) cend.edit = options.edit;
        return await conn.sendMessage(msgObj.from, cend, { quoted: options.quoted });
      } catch (err) {
    ...
    msgObj.react = async (emoji) => {
      try {
        return await conn.sendMessage(msgObj.from, { react: { text: emoji, key: msgObj.key } });
      } catch (err) {
    ...
    // set profile picture (keeps original flow)
    msgObj.setPp = async (jid, buf) => {
      try {
        const img = await makePp(buf);
        if (typeof conn.updateProfilePicture === "function") {
          try { await conn.updateProfilePicture(jidNormalizedUser(jid), img); } catch (e) { /* ignore */ }
        }
    ...
    msgObj.areJidsSame = (jid1, jid2) => areJidsSameUser(jidNormalizedUser(jid1), jidNormalizedUser(jid2));

    // ------------ Memory-safety helpers (preserve API) ------------
    msgObj.discardRaw = () => {
      try {
        delete msgObj.raw;
        delete msgObj.mek;
        if (msgObj.quoted && typeof msgObj.quoted === "object") delete msgObj.quoted.msg;
      } catch (e) {
        /* ignore */
      }
    };
    ...
    // auto-clean heavy fields after CLEANUP_MS (doesn't block execution)
    setTimeout(() => {
      try { msgObj.discardRaw(); } catch (e) {}
    }, CLEANUP_MS);
